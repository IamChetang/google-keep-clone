...
const SelectionBar: React.FC<SelectionBarProps> = ({ selectedCards,sendDataToParent  }) => {
    const { archiveMultipleNotes } = useStore();
    return (
        <React.Fragment>
            {
                selectedCards.length !== 0 ? (
                <AppBar color="default" style={{ zIndex: '2000', position: 'absolute', top: 0, left: 0 }} >
                    <Toolbar>
                        ...
                        <IconButton color="inherit" aria-label="archive">
                            <ArchiveOutlined  onClick={()=> archiveMultipleNotes(selectedCards)}  />
                        </IconButton>
